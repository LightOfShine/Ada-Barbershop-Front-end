...
interface DeleteKapsterModalProps {
  kapster: Kapster;
  onClose: () => void;
  onConfirm: () => void;
}

export function DeleteKapsterModal({ kapster, onClose, onConfirm }: DeleteKapsterModalProps) {
  return (
      ...
      <div className="bg-white rounded-[16px] w-full max-w-sm shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6 flex flex-col items-center text-center">
          ...
          <p className="text-[13px] text-[#6B7280]">
            Apakah Anda yakin ingin menghapus <strong>{kapster.nama}</strong>? Data yang dihapus tidak
            dapat dikembalikan.
          </p>
        </div>
