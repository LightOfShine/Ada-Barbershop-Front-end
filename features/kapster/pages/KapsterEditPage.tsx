      ...
      <div className="bg-white rounded-[20px] p-8 md:p-10 border border-[#E5E7EB]">
        <div className="flex flex-col gap-5 mb-10">
          <div className="relative w-[120px] h-[120px]">
            <div className="w-full h-full rounded-full overflow-hidden relative bg-[#F3F4F6] border-2 border-[#E5E7EB]">
              <Image src={formData.photo} alt="Profile" fill style={{ objectFit: 'cover' }} />
              <div onClick={handlePhotoUploadClick} className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 hover:opacity-100 transition-opacity cursor-pointer text-white">
                <Camera className="w-6 h-6 mb-1" /><span className="text-[10px] font-medium">Ubah Foto</span>
              </div>
            </div>
            <input type="file" id="photo-upload" className="hidden" accept="image/*" />
          </div>
          <h2 className="text-[24px] font-semibold text-[#111827]">{formData.nama}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12 max-w-4xl">
          ...
          <div>
            ...
            <input name="idKapster" value={formData.idKapster} onChange={handleChange} className="w-full h-[44px] px-4 border border-[#E5E7EB] rounded-[8px] text-[15px] font-medium text-[#374151] focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" />
          </div>
          <div>
          ...
          <div>
            ...
            <input name="outletAwal" value={formData.outletAwal} onChange={handleChange} className="w-full h-[44px] px-4 border border-[#E5E7EB] rounded-[8px] text-[15px] font-medium text-[#374151] focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" />
          </div>
          <div>
