...
interface DeleteBarbershopModalProps {
  barbershop: BarbershopItem;
  onClose: () => void;
  onConfirm: () => void;
}

export function DeleteBarbershopModal({ barbershop, onClose, onConfirm }: DeleteBarbershopModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[16px] w-full max-w-sm shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6 flex flex-col items-center text-center">
          ...
          <h3 className="text-[18px] font-semibold text-[#1F2937] mb-2">Hapus Barbershop?</h3>
          <p className="text-[13px] text-[#6B7280]">
            Apakah Anda yakin ingin menghapus <strong>{barbershop.name}</strong>?
            {barbershop.employeeCount > 0 && (
              <> Barbershop ini memiliki <strong>{barbershop.employeeCount} karyawan</strong> yang terkait.</>
            )}
