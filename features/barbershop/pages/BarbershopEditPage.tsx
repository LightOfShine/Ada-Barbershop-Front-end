...
import { INITIAL_BARBERSHOP_DATA, REGION_OPTIONS } from '../constants/mock-data';

export default function BarbershopEditPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  
  // Find mock barbershop based on ID
  const barbershop = INITIAL_BARBERSHOP_DATA.find(b => b.id === params.id) || INITIAL_BARBERSHOP_DATA[0];

  const [formData, setFormData] = useState({
    name: barbershop.name,
    address: barbershop.address,
    regionId: barbershop.regionId,
  });
  ...
  const handleSave = () => {
    ...
    alert('Perubahan barbershop berhasil disimpan!');
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      <div className="text-[15px]">
        ...
        <span className="text-[#1F2937] font-medium">Edit Barbershop</span>
      </div>

      <div className="bg-white rounded-[20px] p-8 md:p-10 border border-[#E5E7EB] max-w-3xl">
        <div className="mb-8">
          <h2 className="text-[20px] font-semibold text-[#111827]">Edit Barbershop: {barbershop.name}</h2>
          <p className="text-[13px] text-[#9CA3AF] mt-1">Ubah informasi detail dari cabang barbershop.</p>
        </div>
